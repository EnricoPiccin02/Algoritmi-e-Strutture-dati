...
Si consideri una funzione di complessità temporale $f(n)$ definita in funzione $n$ della lunghezza dell'input: presumibilmente, al crescere di $n$, ossia al crescere della lunghezza dell'input, aumenta anche il valore della funzione stessa.\\
Le funzioni con cui si sta lavorando sono \textbf{definitivamente positive} e, generalmente, sono infinite per $x \to +\infty$ e vengono studiate per $x \geq 0$.\\
Com'é noto, è stata definita la classe $\Theta(f(n))$ come l'insieme di tutte le funzioni che rassomigliano alla funzione $f(n)$. In questo caso, in particolare, affermare che
\[g(n) = \Theta(f(n))\]
...
in cui $\Theta(f(n))$ è, a tutti gli effetti, una classe di equivalenza, in quanto gode della proprietà di \textbf{riflessività}, \textbf{simmetria} e \textbf{transitività}.\\
...
\[l_{10}(n) \leq \log_{10}(n) < l_{10}(n)\]
in cui $\log_{10}(n)$ viene considerata la lunghezza pratica da considerare come utilizzabile, per quanto differesica dalla lunghezza effettiva. Naturalmente si può scrivere che
\[\frac{1}{2} \cdot l_{10}(n) \leq l_{10}(n) \leq \log_{10}(n) \leq l_{10}(n)\]
per cui si può osservare come $l_{10}(n)$ e $\log_{10}(n)$ appartengono alla stessa classe di equivalenza $\Theta(l_{10}(n))$ in quanto sono state usate le costanti $c_1 = \frac{1}{2}$ e $c_2 = 1$.\\
Quando si impiega la notazione $\Theta$ si ossservi che una funzione $f(n)$ e $\alpha \cdot f(n)$ appartengono alla stessa classe $\Theta$ ed è per questo che normalmente viene omessa la base dei logaritmi, la quale è ininfluente.\\
...
\[3n^2 + 1 \hspace{1em} \frac{n^2}{1000} \hspace{1em} 8945 n^2\]
appartengono alla stessa classe $\Theta(n^2)$.\\
...
\[\lim_{n \to +\infty} \frac{f(n)}{g(n)} = \lim_{n \to +\infty} \sin(n) + 2 = \nexists\]
...
\vspace{1em}
\noindent
\textbf{Notazione}: Si osservi che indicare
\[f(n) = O(g(n))\]
significa affermare che definitivamente si ha che
\[f(n) \leq g(n)\]
...continua...\\
per cui se $f(n)$ è o grande di $g(n)$ e $g(n)$ è o grande $f(n)$ allora $f(n)$ e $g(n)$ appartengono alla stessa classe di equivalenza.
... continua ...\\
Si osservi, inoltre, che quando
...
\end{enumerate}
in cui si ha che
\[\Theta(n) = O \Theta(n \cdot \log(n)) = O \Theta(n^2)\]
ovvero la complessità log-lineare è comunque poco più complessa rispetto a quella lineare, in quanto la crescita all'infinita è molto lenta.
...
nonostante la sua crescita è estremamente lenta rispetto ad una funzione esponenziale normale. Infatti, tali tipologie di funzioni sono impossibili tra trovare nella vita reale, secondo una valenza empirica.
...
\noindent
Natualmente il numero di iterazioni è $k^2$, ma se esse fossero $2 + 2k^2$, la notazione $\Theta$ fa sì che in ogni caso la complessità sia $k^2$ ...continua... anche se il numero di iterazioni è differente, l'orine si mantiene inalterato.

\vspace{1em}
\noindent
\textbf{Osservazione}: Si osservi che nel caso dell'algoritmo \textbf{insertion-sort}, il numero di iterazioni nel caso migliore è pari a $\Theta(n)$, in quanto le iterazioni sono $n-1$, mentre nel caso peggiore la complessità è $\Theta(n^2)$, in quanto le iterazioni sono $\frac{n \cdot (n-1)}{2}$. Per quanto concerne la complessità tipica, ovverosia la complessità media, dal punto di vista empirico si osserva che essa è $\Theta(n^2)$.\\
Per quanto concerne l'algoritmo di Euclide, il numero di iterazioni, nel caso peggiore era di $1 + 2 \cdot \log_2(\min(m,n))$, per cui si ha che la complessità effettiva sia $O(\log(n))$.
...
L'input, come di consueto, è dato da un \textbf{array} $A$ di lunghezza $length(A)=n$, per cui si dovranno ordinare $n$ numeri. Inoltre, due altri input sono $p,r$ le quali sono delle posizioni dell'array intermedie, tali che $1 \leq p \leq r \leq n$
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \State \textbf{if} $p<r$
    \Indent
      \State \textbf{then} $q = \text{parte intera inferiore} \left[\dfrac{p+r}{2}\right]$
...
\noindent
... continua ... In cui la procedura $MERGE$ non è ricorsiva, ma presenta due cicli for. Tale procedura inzia ad operare sue due insiemi di valori che sono già stati ordinati separatamente, con lo scopo di ricavare un unico insieme di valori ordinati, sfruttando l'ordinamento dell'insieme dei valori su cui si sta operando; dopodiché si confrontano tutti i valori in testa, prendendo sempre il più piccolo tra i due che vengono confrontati, fino ad arrivare all'ultimo valore (necessariamente molto elevato) che viene appositamente posto alla fine in modo da segnalare il termine dell'insieme di valori; in particolare si ha che $p < q < r$, in cui i valori che stanno tra $[p-q]$ e $[q-r]$ sono già stati ordinati. La procedura viene esposta di seguito
...
\noindent
In cui le iterazioni coinvolte, naturalmente, fino alla riga $9$ sono
\[n_1 + n_2 = q - p + 1 + r - q = r - p + 1\]
ovverosia il numero delle posizioni che vanno da $p$ a $r$.\\
Mentre dalle righe $9$ in poi il numero di iterazioni è, ovviamente, ancora una volta $r - p + 1$; pertanto il numero di iterazioni complessive è pari al doppio di $r - p + 1$, quindi la complessità è dell'ordine
\[\Theta(r-p+1)\]
...
in cui $n=8$. Per l'ordinamento di tale array, si divide lo stesso in due parti e poi successivamente ogni parte in ancora due parti e nuovamente in due parti fino ad ottenere una divisione dell'ordinamento riducendosi all'unità, in cui i singoli valori saranno automaticamente ordinati. Dopodiché si utilizza la procedure \emph{Merge} per ordinare i blocchi di due, ottenenendo
\[\boxed{4} \hspace{0.5em} \boxed{5} \hspace{0.5em} \boxed{1} \hspace{0.5em} \boxed{3} \hspace{0.5em} \boxed{2} \hspace{0.5em} \boxed{7} \hspace{0.5em} \boxed{0} \hspace{0.5em} \boxed{9}\]
e poi procedendo con i blocchi di $4$, ottenendo
\[\boxed{1} \hspace{0.5em} \boxed{3} \hspace{0.5em} \boxed{4} \hspace{0.5em} \boxed{5} \hspace{0.5em} \boxed{0} \hspace{0.5em} \boxed{2} \hspace{0.5em} \boxed{7} \hspace{0.5em} \boxed{9}\]
e infine si ordina il blocco completo
...
Questa, naturalmente, è una esecuzione parallela, mentre quella dell'algoritmo è lineare, in quanto va ad analizzare progressiamente ogni ramo di un albero binario, risalendolo progresivamente.
...
La sua complessità è \textbf{log-lineare} ed è estremamente ottima in quanto è una complessità generale, che nessun altro algoritmo può batterla. La tecnica impiegata per la risoluzione di tale problema prende il nome di \emph{Divide et impera} (o \emph{Divide and conquer}).
