...
\begin{center}
  23 Marzo 2022
\end{center}
La definizione di probabilità più consona per una valutazione sperimentale è di tipo frequentista: la probabilità di un evento rappresenta il limite, per il numero degli esperimenti che tende all'infinito, del rapporto tra il numero di volte in cui tale evento si è verificato e il numero degli esperimenti effettuati:
\[\boxed{P(E) = \lim_{n \to +\infty} \frac{\#\text{successi}}{n}}\]
Ovviamente è un limite metafisico in quando un numero di esperimenti infinito è materialmente irrealizzabile. Da ciò appare evidente che
\[\boxed{P(\overline{E}) = 1 - P(E)}\]
Considerando, ora, due eventi $E$ e $F$, è nota che la probabilità dell'unione è proprio
\[\boxed{P(E \vee F) = p(E) + p(F) - p(E \wedge F)}\]
Naturalmente, nel caso in cui $E$ e $F$ sono incompatibili, la probabilità diviene additiva, in quanto la probabilità che si verifichino entrambi i due eventi è nulla, allora
\[\boxed{P(E \vee F) = p(E) + p(F)}\]
Per quando concerne la \textbf{probabilità condizionata}, dati due eventi $E$ e $F$, la probabilità che si verifica $E$ dopo che si è verificato $F$ si ottiene che:
\[\boxed{P(E \vert F) = \frac{P(E \wedge F)}{p(F)}}\]
nell'ipotesi in cui l'evento condizionante abbia una probabilità strettamente positiva.\\
Nell'ipotesi in cui $F$ ed $E$ siano indipendenti l'uno rispetto all'altro, è ovvio che
...
ed ecco che si è ottenuta la \textbf{relazione di indipendenza di eventi}:
\[\boxed{P(E \wedge F) = P(A) \cdot P(F)}\]
per cui $E$ non condiziona $F$ \textbf{se e solo se} $F$ non condiziona $F$.\\
Ecco che ora si può considerare $\mathcal{X}$ come \textbf{variabile} (o \textbf{qualità}) \textbf{aleatoria}, formato da uno spazio campionario $\{x_1,...,x_n\}$ di \textbf{eventi elementari} i quali devono essere \textbf{incompatibili} e \textbf{esausitivi}: effettuando un esperimento, si deve sempre verificare uno e uno solo degli eventi elementari. Ciascuno di tali eventi elementari presenta una probabilità $p_i$, ottenendo l'insieme delle probabilità elementari $\{p_1,...,p_n\}$.\\
Nell'ipotesi in cui $\mathcal{X}$ sia un \textbf{numero aleatoria}, ovvero $\mathcal{X} \in \Omega_x \subset \mathbb{R}$, allora ha senso calcolare il suo \textbf{valore medio} (o \textbf{valore atteso}), ovvero $E(\mathcal{X})$.\\
...
\begin{tabularx}{\textwidth}{@{}|P|@{}|P|@{}|P|@{}|P|@{}}
    \hline
    \text{Eventi elementari} & $x_1$ & $x_2$ & $x_3$\\
    \hline
    \text{Probabilità elementari} & $p_1$ & $p_2$ & $p_3$\\
    \hline
\end{tabularx}
\vspace{1em}
...
\noindent
Allora è ovvio che
\[E(\mathcal{X}) = \sum_{i}^n p_i \cdot x_i\]

\vspace{1em}
\subsection{Teorema di trasferimento}
Si consideri una variabile aleatoria $\mathcal{Y}$ che è funzione deterministica di di $\mathcal{X}$, ovvero
\[\boxed{\mathcal{Y} = f(\mathcal{X})}\]
tale per cui $f(a)=f(b)=f(d)=0$, mentre $f(c)=1$. Allora appare evidente come
\[\boxed{E(\mathcal{Y}) = \sum_{y}^n q_y \cdot y}\]
in cui, per quello che si è supposto, si ha $q_0=p_a+p_b+p_d$ e $q_1=p_c$, ottenendo
\[\left(p_a+p_b+p_d\right) \cdot 0 + p_c \cdot 1 = p_a f(a) + p_b f(c) + p_d f(d) + p_c f(c)\]
che corrisponde esattamente a
\[\boxed{E(\mathcal{Y}) = \sum_{x}^n p_x \cdot f(x)}\]
che riscritta in modo alternativo, permette di ottenere la formula del teorema di trasferimento
\[\boxed{E(f(\mathcal{X})) = \sum_{x}^n p_x \cdot f(x)}\]
in cui non si richiede che $\mathcal{X}$ sia di natura numerica. Appliando il teorema del trasferimento a due variabili aleatorie $\mathcal{X}$ e $\mathcal{Y}$ si ottiene
\[E \left[\mathcal{X} + \mathcal{Y}\right] = \sum_x \sum_y p_{xy} \cdot (x + y)\]
ma sfruttando la proprietà distributiva si ottiene
\[E \left[\mathcal{X} + \mathcal{Y}\right] = \sum_x \sum_y p_{xy} \cdot (x + y) = \sum_x \sum_y p_{xy} x + \sum_y \sum_x p_{xy} y = \sum_x x \sum_y p_{xy} x + \sum_y y \sum_x p_{xy}\]
che corrisponde esattemte a
...
\vspace{1em}
\noindent
\textbf{Osservazione}: È noto che la speranza matematica che si verifici la variabile combinatoria $\mathcal{Z}$ è
...
\[P(\mathcal{X}\vert B) = \frac{P(\vert X \cap B)}{P(B)} = \frac{1}{h}\]
